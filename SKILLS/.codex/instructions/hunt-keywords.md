...
## Objective
Search for multiple keywords, IP addresses, SHA256 hashes, or string patterns across all endpoints. The most flexible hunting tool for custom IOC searches, threat intel integration, and ad-hoc investigations.
...
One keyword/IOC per line. Supports filenames, IPs, partial commands, SHA256 hashes, arbitrary strings.

## Pre-Built Keyword Files
125 files in `keywordfiles/` covering: cmd.exe, powershell.exe, wmic.exe, mshta.exe, rundll32.exe, regsvr32.exe, certutil.exe, bitsadmin.exe, cscript.exe, wscript.exe, advpack.dll, comsvcs.dll, TeamViewer.exe, and 100+ more.
...
## Follow-Up
- Hashes found -> `instructions/hunt-hashes.md`
- Network IOCs -> `instructions/hunt-network.md`
- Persistence commands -> `instructions/hunt-persistence.md`
